...
export abstract class DialogComponent {
  ...
  protected getUpdateMethod(): Observable<UpdateResponse> {
    return new Observable<UpdateResponse>(
      (observer: Subscriber<UpdateResponse>) => {
        observer.error({
          error: {
            message:
              'You cannot update these entities, since there exists no PUT endpoint for it',
          },
        });
        observer.complete();
      }
  ...
  public canEditColumn(name: string) {
    if (this.getData().isEdit) {
      return (
        this.updateColumns.filter((x) => x === name).length > 0 &&
  ...
  public upsert() {
    ...
    if (this.getData().isEdit) {
      for (const idx in this.getData().entity) {
        if (
          this.updateColumns.indexOf(idx) === -1 ||
